...
bool loadMedia()
{
	...
	//Open the font
	gFont = TTF_OpenFont( "33_lazy.ttf", 28 );
	if( gFont == NULL )
	{
	...
	SDL_RWops* file = SDL_RWFromFile( "33_nums.bin", "r+b" );
	...
	if( file == NULL )
	{
		...
		file = SDL_RWFromFile( "33_nums.bin", "w+b" );
		if( file != NULL )
		{
...
void close()
{
	...
	SDL_RWops* file = SDL_RWFromFile( "33_nums.bin", "w+b" );
	if( file != NULL )
	{
		//Save data
		for( int i = 0; i < TOTAL_DATA; ++i )
